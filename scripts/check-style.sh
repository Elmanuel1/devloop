...
if ./gradlew tasks --all 2>/dev/null | grep -q "checkstyleMain"; then
    ...
    if [[ -n "$module" ]]; then
        if [[ "$file_path" =~ libs/ ]]; then
            ./gradlew ":libs:${module}:checkstyleMain" --quiet 2>&1 | tail -20
            style_result=${PIPESTATUS[0]}
        else
            ./gradlew ":services:${module}:checkstyleMain" --quiet 2>&1 | tail -20
            style_result=${PIPESTATUS[0]}
        fi
    else
        ./gradlew checkstyleMain --quiet 2>&1 | tail -20
        style_result=${PIPESTATUS[0]}
    fi
