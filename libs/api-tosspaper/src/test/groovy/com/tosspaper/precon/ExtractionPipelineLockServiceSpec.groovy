...
class ExtractionPipelineLockServiceSpec extends Specification {
    ...
    def "TC-L-01: should execute action and return true when per-ID lock is acquired"() {
        given: "the lock is available"
            rLock.tryLock(0L, ExtractionPipelineLockService.EXTRACTION_LOCK_LEASE_MINUTES, TimeUnit.MINUTES) >> true
            rLock.isHeldByCurrentThread() >> true
            def executed = false
            Runnable action = { executed = true }

        when:
            def result = service.tryWithExtractionLock(EXTRACTION_ID, action)
    ...
    def "TC-L-02: should return false and skip action when lock is not acquired"() {
        given: "another instance holds the lock"
            rLock.tryLock(0L, ExtractionPipelineLockService.EXTRACTION_LOCK_LEASE_MINUTES, TimeUnit.MINUTES) >> false
    ...
    def "TC-L-03: should release lock after action completes successfully"() {
        ...
        when:
            service.tryWithExtractionLock(EXTRACTION_ID, { /* no-op */ })
    ...
    def "TC-L-04: should release lock even when action throws a RuntimeException"() {
        ...
        when:
            service.tryWithExtractionLock(EXTRACTION_ID, action)
    ...
    def "TC-L-05: should return false and restore interrupt flag when tryLock is interrupted"() {
        given: "tryLock throws InterruptedException"
            rLock.tryLock(0L, ExtractionPipelineLockService.EXTRACTION_LOCK_LEASE_MINUTES, TimeUnit.MINUTES) >> {
                throw new InterruptedException("interrupted")
            }
            rLock.isHeldByCurrentThread() >> false

        when:
            def result = service.tryWithExtractionLock(EXTRACTION_ID, { /* no-op */ })
    ...
    def "TC-L-06: lock key prefix constant has expected value"() {
        expect:
            ExtractionPipelineLockService.EXTRACTION_LOCK_PREFIX == "extraction:lock:"
    }

    def "TC-L-07: lock lease duration constant has expected value (20 minutes)"() {
        expect:
            ExtractionPipelineLockService.EXTRACTION_LOCK_LEASE_MINUTES == 20L
    }
    ...
    def "TC-L-09: lock key is derived from extraction ID with correct prefix"() {
        given: "a specific extraction ID and a fresh service with a spy on RedissonClient"
            ...
            freshLock.tryLock(0L, ExtractionPipelineLockService.EXTRACTION_LOCK_LEASE_MINUTES, TimeUnit.MINUTES) >> false
            freshLock.isHeldByCurrentThread() >> false
            def freshService = new ExtractionPipelineLockService(freshRedisson)

        when: "trying to lock that extraction"
            freshService.tryWithExtractionLock(specificId, { /* no-op */ })
    ...
    def "TC-L-10: releaseLock unlocks when lock is held by current thread"() {
        ...
        when:
            service.releaseLock(EXTRACTION_ID)
