...
public class DeliveryNoteMapper {
    ...
    public DeliveryNote toDto(DeliveryNotesRecord record) {
        DeliveryNote deliveryNote = new DeliveryNote();
        deliveryNote.setId(record.getId());
        deliveryNote.setExtractionTaskId(record.getExtractionTaskId());
        deliveryNote.setCompanyId(record.getCompanyId());
        deliveryNote.setDocumentNumber(record.getDocumentNumber());
        deliveryNote.setDocumentDate(record.getDocumentDate());
        deliveryNote.setProjectId(record.getProjectId());
        deliveryNote.setProjectName(record.getProjectName());
        deliveryNote.setJobNumber(record.getJobNumber());
        deliveryNote.setPoNumber(record.getPoNumber());
        deliveryNote.setDeliveryMethodNote(record.getDeliveryMethodNote());

        // Parse JSONB party fields to Map<String, Object> for API response
        if (record.getSellerInfo() != null) {
            deliveryNote.setSellerInfo(partyToMap(parseParty(record.getSellerInfo())));
        }
        if (record.getBuyerInfo() != null) {
            deliveryNote.setBuyerInfo(partyToMap(parseParty(record.getBuyerInfo())));
        }
        if (record.getShipToInfo() != null) {
            deliveryNote.setShipToInfo(partyToMap(parseParty(record.getShipToInfo())));
        }
        if (record.getBillToInfo() != null) {
            deliveryNote.setBillToInfo(partyToMap(parseParty(record.getBillToInfo())));
        }
        ...
        if (record.getLineItems() != null) {
            try {
                ...
                deliveryNote.setLineItems(convertLineItems(extractionLineItems));
            } catch (Exception e) {
                log.warn("Failed to parse lineItems for delivery note {}", record.getId(), e);
            }
        }
        ...
        if (record.getShipmentDetails() != null) {
            try {
                deliveryNote.setShipmentDetails(parseShipmentDetailsOpenApi(record.getShipmentDetails()));
            } catch (Exception e) {
                log.warn("Failed to parse shipmentDetails for delivery note {}", record.getId(), e);
            }
        }
        ...
        if (record.getDeliveryAcknowledgement() != null) {
            try {
                deliveryNote.setDeliveryAcknowledgement(parseDeliveryAckOpenApi(record.getDeliveryAcknowledgement()));
            } catch (Exception e) {
                log.warn("Failed to parse deliveryAcknowledgement for delivery note {}", record.getId(), e);
            }
        }

        deliveryNote.setCreatedAt(record.getCreatedAt() != null ? record.getCreatedAt() : null);
        deliveryNote.setCreatedBy(record.getCreatedBy());
        deliveryNote.setStatus(record.getStatus() != null ? DeliveryNoteStatus.fromValue(record.getStatus()) : null);
        deliveryNote.setPurchaseOrderId(record.getPurchaseOrderId());

        return deliveryNote;
    }

    public List<DeliveryNote> toDtoList(List<DeliveryNotesRecord> records) {
    ...
    /**
     * Convert DeliveryNotesRecord to DeliveryNote domain model.
     */
    public com.tosspaper.models.domain.DeliveryNote toDomain(DeliveryNotesRecord record) {
        if (record == null) {
            return null;
        }
        return com.tosspaper.models.domain.DeliveryNote.builder()
    ...
    /**
     * Convert domain DeliveryNote to generated OpenAPI DeliveryNote.
     * Maps from domain model (extraction DTOs) to generated API model.
     */
    public DeliveryNote toDto(com.tosspaper.models.domain.DeliveryNote domain) {
        if (domain == null) {
            return null;
        }
        
        DeliveryNote dto = new DeliveryNote();
    ...
    /**
     * Convert domain Status enum to generated DeliveryNoteStatus enum.
     */
    private DeliveryNoteStatus convertStatus(com.tosspaper.models.domain.DeliveryNote.Status domainStatus) {
        if (domainStatus == null) {
            return null;
        }
        // Map domain status values to generated status values
        return switch (domainStatus) {
            case DRAFT -> DeliveryNoteStatus.DRAFT;
            case ACCEPTED -> DeliveryNoteStatus.DELIVERED;
        };
    }

    private com.tosspaper.models.domain.DeliveryNote.Status findStatusByValue(String statusValue) {
        for (com.tosspaper.models.domain.DeliveryNote.Status status : com.tosspaper.models.domain.DeliveryNote.Status.values()) {
