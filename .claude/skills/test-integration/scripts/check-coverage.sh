...
if [[ -n "$module" ]]; then
    log "Running JaCoCo for module: $module"
    ./gradlew ":libs:${module}:test" ":libs:${module}:jacocoTestReport" --quiet 2>&1
    test_result=$?
    REPORT_PATH="libs/${module}/build/reports/jacoco/test/html/index.html"
    XML_REPORT="libs/${module}/build/reports/jacoco/test/jacocoTestReport.xml"
else
