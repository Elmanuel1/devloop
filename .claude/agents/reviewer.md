...
2. **Load design skills** — read these skill files:
   - `.claude/skills/design-checklist/SKILL.md` — architecture checklist + 28-point gate
   - `.claude/skills/solid-principles/SKILL.md` — SOLID violations
   - `.claude/skills/dry-patterns/SKILL.md` — duplication
   - `.claude/skills/testability/SKILL.md` — testability issues
   - `.claude/skills/refactoring-checklist/SKILL.md` — KISS, coupling, exceptions, final audit
...
- [ ] **No unnecessary abstractions** — interface with only one implementation and no plan for a second?
- [ ] **No passthrough wrappers** — delegate class that just forwards every call?
- [ ] **No overkill patterns** — Strategy/Factory/Builder used for 2 or fewer cases?
- [ ] **No single-use helpers** — utility class used in exactly one place?
- [ ] **No over-configuration** — config for values that never change?
...
### ci:passed
- Fetch the PR diff yourself: `gh pr diff {prNumber} --repo Build4Africa/tosspaper`
- Read every changed file in full
- Run the full review checklist (sections A-G + KISS)
- Post findings to the PR via `gh_review.py`

The orchestrator just passes you `{event, prNumber, issueKey}` — you do the rest.
